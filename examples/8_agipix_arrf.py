#!/usr/bin/env python
"""
Agipix ARRF (Stereo cam0/cam1 + LiDAR + PX4 + ROS2) standalone example for Isaac Sim 6.0.
"""
...
class AgipixApp:
    def __init__(self, namespace="drone", vehicle_id=0):
        ...
        self.isaac_imu = None
        self._cb_physics = None

        self._setup_world()
        self._spawn_vehicle()
        self._setup_publishers_and_sensors()

        # Register physics step callback with SimulationManager
        self._cb_physics = SimulationManager.register_callback(
            self.physics_step, SimulationEvent.PHYSICS_POST_STEP
        )
        ...
        config_multirotor.graphical_sensors = [
            MonocularCamera(
                ...
                config={
                    ...
                    "intrinsics": np.array([
                        [606.3120727539062, 0.0, 314.6913146972656],
                        [0.0, 605.92626953125, 252.1909942626953],
                        [0.0, 0.0, 1.0],
                    ]),
                    "resolution": (640, 400),
                    "frequency": 30,
    ...
    def run(self):
        ...
        self.drone.stop()
        if self._cb_physics is not None:
            try:
...
def parse_cli_args():
    parser = argparse.ArgumentParser(description="Run the Agipix ARRF standalone simulation example.")
    parser.add_argument(
        "--namespace",
        default="drone",
